-- Clean SQL Script to Find Long Filenames in Fish-Smart Database
...
FROM (
    SELECT ImageUrl as FilePath FROM Backgrounds WHERE ImageUrl IS NOT NULL AND LEN(ImageUrl) > 150
    UNION ALL
    SELECT PhotoUrl FROM Catches WHERE PhotoUrl IS NOT NULL AND LEN(PhotoUrl) > 150
    UNION ALL  
    SELECT FileName FROM CategoryFiles WHERE FileName IS NOT NULL AND LEN(FileName) > 150
) as AllLongFiles;