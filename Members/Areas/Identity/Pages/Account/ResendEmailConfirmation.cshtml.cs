        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!string.IsNullOrEmpty(callbackUrl))
            {
                await _emailSender.SendEmailAsync(
                    Input.Email,
                    $"Reminder: Confirm Your {siteName} Email",
                    $"<!DOCTYPE html>" +
                    "<html lang=\"en\">" +
                    "<head>" +
                    "    <meta charset=\"UTF-8\">" +
                    "    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">" +
                    $"    <title>Reminder: Confirm Your Email - {siteName}</title>" +
                    "</head>" +
                    "<body style=\"font-family: sans-serif; line-height: 1.6; margin: 20px;\">" +
                    "    <p style=\"margin-bottom: 1em;\">Dear Member,</p>" +
                    $"    <p style=\"margin-bottom: 1em;\">We noticed you haven't confirmed your email address for your {siteName} Cloud Site account yet. To activate your account, please click the button below:</p>" +
