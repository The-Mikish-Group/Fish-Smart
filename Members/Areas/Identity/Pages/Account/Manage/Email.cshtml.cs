        ...
        ILogger<EmailModel> logger) : PageModel
    {
        ...
        public async Task<IActionResult> OnPostChangeEmailAsync()
        {
            ...
            if (Input?.NewEmail != null && Input.NewEmail != email)
            {
                ...
                if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SITE_NAME_FISH_SMART")))
                {
                ...
                if (callbackUrl != null)
                {
                    await _emailSender.SendEmailAsync(
                        Input.NewEmail,
                        $"{siteName} - Confirm Your Email Address Change",
                        $"<!DOCTYPE html>" +
                        "<html lang=\"en\">" +
                        "<head>" +
                        "    <meta charset=\"UTF-8\">" +
                        "    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">" +
                        $"    <title>Confirm Your New Email - {siteName}</title>" +
                        "</head>" +
                        "<body style=\"font-family: sans-serif; line-height: 1.6; margin: 20px;\">" +
                        "    <p style=\"margin-bottom: 1em;\">Dear Member,</p>" +
                        $"    <p style=\"margin-bottom: 1em;\">You are receiving this email because you requested to change the email address associated with your {siteName} account.</p>" +
        ...
        public async Task<IActionResult> OnPostSendVerificationEmailAsync()
        {
            ...
            await _emailSender.SendEmailAsync(
                ...
                "<body style=\"font-family: sans-serif; line-height: 1.6; margin: 20px;\">" +
                "    <p style=\"margin-bottom: 1em;\">Dear Member,</p>" +
                $"    <p style=\"margin-bottom: 1em;\">Thank you for registering with the {siteName} Cloud Site!</p>" +
                ...
                "        </a>" +
                "    </div>" +
                $"    <p style=\"margin-bottom: 1em;\">This email verification link is valid for a limited time. If you did not register for an account with {siteName}, you can disregard this email.</p>" +
                "    <p style=\"margin-bottom: 0;\">Thank you,</p>" +
                $"    <p style=\"margin-top: 0;\">The {siteName} Team<img src=\"https://{siteName}.com/Images/LinkImages/SmallLogo.png\" alt=\"{siteName} Logo\" style=\"vertical-align: middle; margin-left: 3px; height: 35px;\"></p>" +
                "</body>" +
                "</html>"
