...
namespace Members.Areas.Identity.Pages
{
    public class EditUserModel(UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager, IEmailSender emailSender, ApplicationDbContext dbContext) : PageModel
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("OnPostAsync (default) called");
        ...
        {
            System.Diagnostics.Debug.WriteLine($"OnPostDeleteAsync called - Input.Id: {Input.Id}");
            ...
            {
                System.Diagnostics.Debug.WriteLine($"User not found with ID: {Input.Id}");
            ...
            System.Diagnostics.Debug.WriteLine($"Found user to delete: {userToDelete.UserName} ({userToDelete.Id})");
            ...
            // Delete the user
            System.Diagnostics.Debug.WriteLine($"Attempting to delete user: {userToDelete.UserName}");
            var result = await _userManager.DeleteAsync(userToDelete);
            System.Diagnostics.Debug.WriteLine($"Delete result succeeded: {result.Succeeded}");
            
            if (!result.Succeeded)
            {
                System.Diagnostics.Debug.WriteLine("Delete failed with errors:");
                foreach (var error in result.Errors)
                {
